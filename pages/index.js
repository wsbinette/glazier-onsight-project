...
const projectsFormatter = (projectData) => {
  const projectInstance = projectData.projects.projData.map((project) => {
    return {
      id: project.id,
...
const GlazierHome = ({data}) => {
  const [sidebarOpen, setSidebarOpen] = useState(false)
  const glazierData = data;
  const [projects, setProjects] = useState([])
  const pinnedProjects = projects.filter((project) => project.status == 'installation scheduled' ? project : null)

  useEffect(() => {
    setProjects(projectsFormatter(glazierData))
  }, [])
      ...
      <div className="min-h-full">
        ...
        <div className="hidden lg:flex lg:flex-col lg:w-64 lg:fixed lg:inset-y-0 lg:border-r lg:border-gray-200 lg:pt-5 lg:pb-4 lg:bg-gray-100">
          <div className="flex items-center flex-shrink-0 px-6">
            <h1 className='font-bold text-3xl'>Glazier</h1>
          </div>
                  ...
                  <li key={project.id} className="relative col-span-1 flex shadow-sm rounded-md">
                    ...
                    <div className="flex-1 flex items-center justify-between border-t border-r border-b border-gray-200 bg-white rounded-r-md truncate">
                      <div className="flex-1 px-4 py-2 text-sm truncate">
                        ...
                        <p className="text-gray-500">{new Date(project.created_date).toLocaleDateString()}</p>
                      </div>
                  ...
                  <thead>
                    <tr className="border-t border-gray-200">
                      <th
                        ...
                        scope="col"
                      >
                        <span className="lg:pl-2">Active Projects</span>
                      </th>
                      <th
                        className="px-6 py-3 border-b border-gray-200 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                        scope="col"
                        ...
                        <td className="px-6 py-3 max-w-0 w-full whitespace-nowrap text-sm font-medium text-gray-900">
                          <div className="flex items-center space-x-3 lg:pl-2">
                            ...
                            <a href="#" className="truncate hover:text-gray-600">
                              <span>
                                {project.customer.name} <span className="text-gray-500 font-normal">in {project.customer.city}, {project.customer.state}</span>
                              </span>
                            </a>
                          </div>
                        </td>
...
export async function getServerSideProps(){
  const supRes = await fetch('https://62b78ab6691dcea2733fe5d5.mockapi.io/api/v1/supplier')
  const projRes = await fetch('https://62b78ab6691dcea2733fe5d5.mockapi.io/api/v1/projects')
